    ...
    <div className="p-8">
      ...
      <div className="card mb-6">
        ...
        <div className="flex gap-4 mt-4">
          ...
          <Link to="/gamification" className="text-sm text-primary-400 hover:text-primary-300">
            Voir les quêtes →
          </Link>
        </div>
      </div>
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            <StatCard
              ...
              icon={TrendingDown}
              color="bg-red-500"
              sub={`${summary.days_passed}/${summary.days_in_month} jours`}
            />
            <StatCard
              label="Restant"
              value={formatCurrency(summary.remaining)}
              icon={Zap}
              color={summary.remaining >= 0 ? 'bg-primary-600' : 'bg-orange-500'}
            />
            <StatCard
              label="Score santé"
          ...
          <div className="card mb-6">
            ...
            <div className="flex items-center gap-6">
              <div>
                ...
                <p className="text-xl font-bold text-white">{formatCurrency(summary.projected_total)}</p>
              </div>
              <div>
                ...
                <p className={`text-xl font-bold ${summary.projected_remaining >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                  {formatCurrency(summary.projected_remaining)}
                </p>
              </div>
              {summary.savings_goal > 0 && (
                <div>
                  <p className="text-sm text-gray-400">Objectif épargne</p>
                  <p className="text-xl font-bold text-gold-400">{formatCurrency(summary.savings_goal)}</p>
                </div>
              )}
            </div>
          </div>
              ...
              {summary.budget_categories?.map((cat) => {
                ...
                return (
                  <div key={cat.id}>
                    ...
                    <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all duration-300 ${over ? 'bg-red-500' : 'bg-primary-500'}`}
                        style={{ width: `${pct}%` }}
                      />
                    </div>
                  </div>
