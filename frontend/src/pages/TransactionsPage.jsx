...
export default function TransactionsPage() {
  ...
  const categories = budgetData?.budget?.budget_categories || [];
      ...
      <div className="space-y-2">
        {txData?.transactions?.length === 0 && (
          <div className="card text-center py-8 text-gray-500">
            Aucune transaction. Commence à enregistrer tes dépenses !
          </div>
        )}
        {txData?.transactions?.map((tx) => (
          <div key={tx.id} className="card py-3 flex items-center gap-4">
            ...
            <button onClick={() => deleteTx.mutate(tx.id)} className="text-gray-600 hover:text-red-400 transition-colors ml-2">
              <Trash2 size={15} />
            </button>
          </div>
        ))}
